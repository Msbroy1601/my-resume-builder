      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-5">
            ...
            {/* No resume data notice */}
            {!resumeData && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
                <span className="text-xl flex-shrink-0">💡</span>
                <div>
                  <p className="text-sm font-semibold text-amber-800">Resume data not found</p>
                  <p className="text-xs text-amber-600 mt-0.5">
                    <button onClick={() => navigate('/builder')} className="underline">Fill your resume first</button> and we'll pre-fill your name, skills and experience here automatically.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Job title you're applying for <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    value={targetJob}
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    ...
                    <div>
                      <p className={`text-sm font-bold ${tone === t.id ? 'text-blue-700' : 'text-gray-800'}`}>{t.label}</p>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <p className="text-xs text-gray-500 mb-4">Pick up to 4 to highlight in your letter</p>
              ...
              {selectedStrengths.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedStrengths.map(s => (
                    <span key={s} className="flex items-center gap-1 bg-blue-100 text-blue-700 text-xs font-semibold px-3 py-1 rounded-full">
                      {s}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              {resumeData ? (
                <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-600 space-y-0.5">
                  ...
                  <button onClick={() => navigate('/builder')} className="text-blue-500 hover:underline text-xs mt-1 block">
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
              ...
              {/* Letter preview — this is what gets printed */}
              <div
              ...
              >
                {/* Sender info */}
                <div className="mb-8">
                  <p className="text-xl font-bold text-gray-900" style={{fontFamily: 'Georgia, serif'}}>{name || 'Your Name'}</p>
                ...
                {(company || targetJob) && (
                  <div className="mb-6">
                    {company && <p className="text-sm font-semibold text-gray-800">{company}</p>}
                    {targetJob && <p className="text-sm text-gray-500">Re: {targetJob} position</p>}
                  </div>
                )}

                {/* Greeting */}
                <p className="text-sm text-gray-800 mb-4">{letter.greeting}</p>

                {/* Opening paragraph */}
                <p className="text-sm text-gray-700 leading-relaxed mb-4">{letter.opener}</p>
                ...
                {/* Closing */}
                <p className="text-sm text-gray-700 leading-relaxed mb-8">{letter.closing}</p>

                {/* Sign-off */}
                <p className="text-sm text-gray-700">{letter.sign}</p>
                <p className="text-sm font-bold text-gray-900 mt-2">{name || 'Your Name'}</p>
              </div>
            </div>
          </div>
